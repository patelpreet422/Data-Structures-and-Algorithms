...
Such a partial pivoting may be required if, at the pivot place, the entry of the
matrix is zero. In any case, choosing the largest possible absolute value of the
pivot improves the numerical stability of the algorithm, when floating point is
...
template <typename T> void gaussJordan(std::vector<std::vector<T>> &eqns) {
...
template <typename T> void print_mat(const vector<vector<T>> &mat) {
  for (auto &r : mat) {
    cout << r << '\n';
  }
}

template <typename T> vector<T> solveLinearSystem(vector<vector<T>> eqn) {
...
int main() {
  vector<vector<float>> mat{{2, 3, 1, 5}, {1, 1, 1, 10}, {1, 1, 2, 20}};
