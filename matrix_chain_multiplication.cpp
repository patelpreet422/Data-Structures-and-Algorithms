    ...
    for(int d = 1; d < n; ++d)
    {
        for(int i = 0; i < n-d; ++i)
        {
            ...
            for(int k = i; k < col; ++k)
            {
                ...
                if(M[i][k] + M[k+1][col] + dims[i]*dims[k+1]*dims[col+1] < M[i][col])
                {
                    M[i][col] = M[i][k] + M[k+1][col] + dims[i]*dims[k+1]*dims[col+1];

                    // Since we come here means that A[k] is the slicing point so we store this point
                    S[i][col] = k;
                }
                //M[i][col] = min(M[i][col], M[i][k] + M[k+1][col] + dims[i]*dims[k+1]*dims[col+1]);
                cout << " + dim[" << i << "]*" << "dim[" << k+1 << "]*" << "dim[" << col+1 << "]\n";
            }
            cout << "\n";
        }
    }
