...
bool isSafe(vector<vector<bool>>& chess, int r, int c) {
  if(r > chess.size()) return false;
  if(r < 0) return false;
  if(c > chess.size()) return false;
  if(c < 0) return false;
  ...
  for(auto&& d: dirs) {
    ...
    for(int k = 1; ((0 <= r+k*x) && (r+k*x < chess.size())) && ((0 <= c+k*y) && (c+k*y < chess.size())); ++k) {
      // cout << r+k*x << ", " << c+k*y << '\n';
      if(chess[r+k*x][c+k*y] == true) return false;
    }
  }
  return true;
}
