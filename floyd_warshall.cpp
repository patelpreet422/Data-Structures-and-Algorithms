...
struct Graph
{
    ...
    void add_edge(vertex_t v1, vertex_t v2, weight_t weight = 1, bool directed = true)
    {
        ...
        if(!directed) adjacency_matrix[v2][v1] = weight;
    }
...
adjacency_matrix_t floyd_warshall(Graph g, vertex_t s, vertex_t t)
{
    ...
    vector<vector<vertex_t>> parent(g.vertices, vector<vertex_t>(g.vertices, numeric_limits<int>::max()));
    ...
    for(vertex_t i = 0; i < g.vertices; ++i)
    {
        ...
        {
            if(g.adjacency_matrix[i][j] != 0) parent[i][j] = j;
            if(path_matrix[i][j] == 0) path_matrix[i][j] = numeric_limits<int>::max();
            if(i == j) path_matrix[i][j] = 0;
        }
    }
    ...
    // print path
    cout << s << " ";
    for(vertex_t current_vertex = parent[s][t]; current_vertex != numeric_limits<int>::max(); 
