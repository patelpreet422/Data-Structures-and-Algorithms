...
struct Base {
  ...
  virtual void bf() { cout << "Base\n"; }
};
struct Derived : public Base {
  // all the paths through which Derived can be constructed are deleted so in
  // theory you can't create object of Derived
  Derived() = delete;
  Derived(const Derived &) = delete;
  Derived &operator=(const Derived &) = delete;
  Derived &operator=(Derived &&) = delete;

  void df() { cout << "Derived\n"; }
};
int main() {
  Base *b = new Base;
