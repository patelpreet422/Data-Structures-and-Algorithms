...
template <typename T>
int LCSS_r(T&& c1, T&& c2, size_t s1, size_t s2) {
    if (s1 == 0 || s2 == 0) return 0;
    if (c1[s1 - 1] == c2[s2 - 1]) return 1 + LCSS(c1, c2, s1-1, s2-1);
    else return max(LCSS(c1, c2, s1, s2-1), LCSS(c1, c2, s1-1, s2));
}

template <typename T>
int LCSS_r(T&& c1, T&& c2) {
...
int LCSS(T&& s1, T&& s2)
{
    ...
    for (int i = 1; i <= s2.size(); ++i) {
        for (int j = 1; j <= s1.size(); ++j) {
            dp_table[i][j] = ((s2[i-1] == s1[j-1]) ? dp_table[i-1][j-1]+1 : max(dp_table[i][j-1], dp_table[i-1][j]));
        }
    }
    ...
    while (i > 0 && j > 0)
    {
        ...
        if (s2[i-1] == s1[j-1])
        {
            ...
            i--; j--; lcss--;     // reduce values of i, j and index
        }
