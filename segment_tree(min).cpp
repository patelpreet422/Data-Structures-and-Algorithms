...
struct segment_tree {
  ...
  void update(int64_t n) {
    data[n] =
        min(data[2 * n] + delta[2 * n], data[2 * n + 1] + delta[2 * n + 1]);
  }
  ...
  int64_t query(int64_t l, int64_t r) { return query(1, l, r); }
  ...
  void increment(int64_t l, int64_t r, int64_t del) { increment(1, l, r, del); }
};
