...
tuple<Point, Point, double> naiveClosestPair(Point *points, int n, Point &p1, Point &p2)
{
...
tuple<Point, Point, double> closestPair(Point *points, int low, int high, Point *strip, Point &p1, Point &p2)
{
	int n = (high-low)+1;
	if(n <= 3)return naiveClosestPair(points, n, p1, p2);
	...
	sort(strip, strip+j, [](const Point &p1, const Point &p2){return p1.y < p2.y;});
...
tuple<Point, Point, double> closestPair(Point *points, int n)
{
	Point p1, p2;
	Point strip[n];
	sort(points, points+n, [](const Point &p1, const Point &p2){return p1.x < p2.x;});
...
int main()
{
	...
	cout << get<0>(closestPair(points, n)) << ", " << get<1>(closestPair(points, n)) << ": " << get<2>(closestPair(points, n)) << '\n';
