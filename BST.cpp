    ...
    bool insert(T elem) {
        ...
        while (true) {
            ...
            if (elem < currHead->data) {
                parent = currHead;
                if (currHead->hasLeftChild()) currHead = currHead->left;
                else {
                ...
                parent = currHead;
                if (currHead->hasRightChild()) currHead = currHead->right;
                else {
                    Node<T> *temp = new Node<T>(elem);
                    temp->parent = parent;
                    currHead->right = temp;
...
//        return false;
    ...
    bool contain(T elem) {
        Node<T> *currHead = root;
        while (true) {
            if(currHead->data == elem) return true;
            if (elem < currHead->data) {
                if(currHead->hasLeftChild()) currHead = currHead->left;
                else return false;
            } else {
    ...
    void remove(T elem, Node<T>* node) {
        ...
        if(elem < node->data) remove(elem, node->left);
        else if(elem > node->data) remove(elem, node->right);
        else{
            if(node->isLeaf()){
...
//                    only one child
                    if(node->data <= node->parent->data) node->parent->left = node->hasLeftChild() ? node->left : node->right;
                    else node->parent->right = node->hasLeftChild() ? node->left : node->right;
                    --no_of_elements;
                    return;
                } else {
